...
class PU_train_data(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if idx % 2 == 1:  # The examples with odd indices are noises (considered to be positive examples)
            noise_wav, _ = torchaudio.load(self.noise_fnames[idx // 2],
                                           frame_offset=int(16000 * self.start_times[idx // 2]),
                                           num_frames=self.wav_len)
            noise_wav = (noise_wav - torch.mean(noise_wav)) / torch.std(noise_wav)
            noise_wav = torch.squeeze(noise_wav)
            ...
            noise_wav, _ = torchaudio.load(self.noise_fnames2[idx // 2],
                                           frame_offset=int(16000 * self.start_times2[idx // 2]),
                                           num_frames=self.wav_len)
            noise_wav = (noise_wav - torch.mean(noise_wav)) / torch.std(noise_wav)
            noise_wav = torch.squeeze(noise_wav)
            ...
            # scale clean speech
            factor = 10. ** (self.SNRs[idx // 2] / 20.) / torch.sqrt(
                torch.sum(clean_wav ** 2) / torch.sum(noise_wav ** 2))
            clean_wav = clean_wav * factor
